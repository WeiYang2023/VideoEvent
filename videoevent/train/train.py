...
from videoevent.train.llava_trainer import LLaVATrainer

from videoevent import conversation as conversation_lib
...
class ModelArguments:
    ...
    lora_resume: bool = field(default=False)  # 是否从二阶段lora获取结果
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            if role == conv.roles[0]:
                ...
                # )
                guided_sent = sentence["value"].replace("<image>\n", "")
                guided_sent = guided_sent.replace("\n<image>", "")
                if refine_prompt:
...
class LazySupervisedDataset(Dataset):
    ...
    def frame_sample(self, duration, mode="uniform", local_fps=None):
        if mode == "uniform":
            # Calculate the size of each segment from which a frame will be extracted
            seg_size = float(duration - 1) / self.num_frames
            ...
            segment_len = min(local_fps // 1, duration)
        ...
        while attempt < max_attempt:
            try:
                ...
                if "image" in sources[0]:
                    ...
                    if "instruction" in sources[0].keys():
                        instruction = sources[0]["instruction"]
                    ...
                    if suffix == "pkl":
                        ...
                        video_gif = imageio.get_reader(video_file)
                        duration, local_fps = len(video_gif), 10

                        segment_len = min(local_fps // 1, duration)
                        frame_idx = np.arange(
                            segment_len // 2, duration, segment_len, dtype=int
                        )
                        total_frames = len(frame_idx)

                        if total_frames > self.num_frames:
                            frame_id_list = self.frame_sample(
                                duration, mode="uniform", local_fps=local_fps
                            )
                        else:
                            frame_id_list = frame_idx

                        video = [
                            frame
                            for index, frame in enumerate(video_gif)
                            if index in frame_id_list
                        ]
                        processor = self.data_args.image_processor
                        ...
                        video = video_frames[frame_id_list]
                        processor = self.data_args.image_processor
                        image = processor.preprocess(video, return_tensors="pt")[
                            "pixel_values"
                        ]
                        ...
                        decord_vr = VideoReader(
                            uri=video_file, ctx=cpu(0), num_threads=1
                        )
                        duration, local_fps = len(decord_vr), float(
                            decord_vr.get_avg_fps()
                        )
...
def train():
    ...
    model.get_model().initialize_cross_attention(
        model_args, stage=training_args.stage, vs=2
    )  # 1
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            print(name, param.dtype, param.device, param.numel())
